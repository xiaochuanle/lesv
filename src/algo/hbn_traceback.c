...
HbnTracebackDataNew()
{
    ...
    data->ksw->zdrop = 40;
    data->semi_align = ApproxSemeGappedAlignDataNew();
...
HbnTracebackDataFree(HbnTracebackData* data)
{
    ...
    ApproxSemeGappedAlignDataFree(data->semi_align);
    free(data);
    return NULL;
}
    ...
    const BOOL do_traceback)
{
    int qoff, qend, soff, send;
    double ident_perc;
    int r = dalign_align(data->dalign,
                query,
                query_gapped_start,
                query_length,
                subject,
    ...
    if (!do_traceback) {
        ...
        data->soff = soff;
        data->send = send;
        data->ident_perc = ident_perc;
        return 1;
    }

    r = edlib_nw(data->edlib,
            query + qoff,
            qend - qoff,
            subject + soff,
            send - soff,
            ...
            &data->sabuf);
    if (!r) return r;
    r = truncate_align_bad_ends(ks_s(data->qabuf),
            ks_s(data->sabuf),
            ks_size(data->qabuf),
            &data->qoff,
            &data->qend,
            ...
            (const char**)&data->sae);
    if (!r) return r;
    data->ident_perc = calc_ident_perc(data->qas, data->qae, data->qae - data->qas, NULL, NULL);
    validate_aligned_string(__FILE__, __FUNCTION__, __LINE__,
        0, query, data->qoff, data->qend, data->qas,
        0, subject, soff, send, data->sas,
