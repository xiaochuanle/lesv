    ...
    vec_align_tag* align_tag_list)
{
...
#if 0
    int r = (qaln[0] == GAP_CHAR)
            ||
            (taln[0] == GAP_CHAR)
            ||
            (qaln[aln_size-1] == GAP_CHAR)
            ||
            (taln[aln_size-1] == GAP_CHAR);
    if (r) {
        hbn_fwrite(qaln, 1, aln_size, stderr);
        fprintf(stderr, "\n");
        for (int i = 0; i < aln_size; ++i) {
            if (qaln[i] == taln[i]) fprintf(stderr, "|");
            else fprintf(stderr, "*");
        }
        fprintf(stderr, "\n");
        hbn_fwrite(taln, 1, aln_size, stderr);
        fprintf(stderr, "\n");
        exit(0);
    }
#endif
    ...
    for (size_t p = 0; p < aln_size; ++p) {
        ...
        if (jj >= ALIGN_TAG_MAX_DELTA || p_jj >= ALIGN_TAG_MAX_DELTA) {
            HBN_LOG("long delta at i = %d,j = %d", i, j);
            continue;
        }
