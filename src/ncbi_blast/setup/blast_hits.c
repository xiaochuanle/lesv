...
void
HbnHSPResultsClear(HbnHSPResults* results, int qidx_from, int qidx_to)
{
...
{
   SmallObjectAllocStat(results->pointer_alloc);
   SmallObjectAllocStat(results->hsp_alloc);
   SmallObjectAllocStat(results->hsplist_alloc);
   HBN_LOG("aligned_string_len: %zu / %zu", results->aligned_strings.l, results->aligned_strings.m);
   HBN_LOG("output buffer len: %zu / %zu", results->output_buf.l, results->output_buf.m);
}
