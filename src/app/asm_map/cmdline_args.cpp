...
const int kDfltMemScKmerSize = 19;
...
const int kDfltAsmMapMaxTargetSeqs = 100;
const int kDfltAsmMapMaxHspsPerSubject = 4;
...
void CommandLineArguments::SetArgumentDescriptions(CArgDescriptions& arg_desc)
{
    ...
    arg_desc.AddOptionalKey(kArgMaxTargetSequences, "num_sequences",
                ...
                "Default = '" + NStr::IntToString(kDfltAsmMapMaxTargetSeqs) + "'",
                CArgDescriptions::eInteger);
...
void Init_HbnProgramOptions(HbnProgramOptions* opts, const EHbnTask task)
{
    ...
    opts->max_hsps_per_subject = kDfltAsmMapMaxHspsPerSubject;
    opts->hitlist_size = kDfltAsmMapMaxTargetSeqs;
...
s_PreCheckCmdLineArgs(int argc, char* argv[], CArgDescriptions* arg_desc)
{
    ...
    string kHbnUsage = kProgram + " [OPTIONS] db_dir sr_dir";
