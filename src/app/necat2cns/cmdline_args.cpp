...
const int kDfltMemScKmerSize = 15;
const int kMemScKmerSizeMin = 8;
const int kMemScKmerSizeMax = 32;
...
const double kDfltOvlpCovPerc = 80.0;
const string kArgOvlpCovRes("ovlp_cov_res");
const int kDfltOvlpCovRes = 5000;
const string kArgMinCov("min_cov");
const int kDfltMinCov = 4;
const string kArgMinSize("min_size");
const int kDfltMinSize = 2000;
const double kDfltPercIdentity = 65.0;
const string kArgRefineOvlp("refine_ovlp");
const bool kDfltRefineOvlp = false;
const string kArgUseKsw("use_ksw");
const bool kDfltUseKsw = false;
const string kArgUseReplaceMode("use_replace_mode");
const bool kDfltUseReplaceMode = false;
...
void CommandLineArguments::SetArgumentDescriptions(CArgDescriptions& arg_desc)
{
    ...
    arg_desc.AddFlag(kArgRefineOvlp, "Refine overlaps with 2-piece gap-cost Smith-Waterman algorithm", true);
    ...
    arg_desc.AddFlag(kArgUseKsw, "Use KSW to perform alignment", true);

    arg_desc.AddFlag(kArgUseReplaceMode, "Replace subsequence with corrected one", true);
...
void CommandLineArguments::ExtractAlgorithmOptions(const CArgs& args, CBlastOptions& options)
{
    ...
    if (args.Exist(kArgRefineOvlp)) {
        m_Options->refine_ovlp = static_cast<bool>(args[kArgRefineOvlp]);
    }
    ...
    if (args.Exist(kArgUseKsw)) {
        m_Options->use_ksw = static_cast<bool>(args[kArgUseKsw]);
    }

    if (args.Exist(kArgUseReplaceMode)) {
        m_Options->use_replace_mode = static_cast<bool>(args[kArgUseReplaceMode]);
    }
...
void Init_HbnProgramOptions(HbnProgramOptions* opts)
{
    ...
    opts->refine_ovlp = kDfltRefineOvlp;
    ...
    opts->use_ksw = kDfltUseKsw;
    opts->use_replace_mode = kDfltUseReplaceMode;
...
char* HbnProgramOptions2String(const HbnProgramOptions* opts)
{
    ...
    if (opts->refine_ovlp) os_one_flag_option(kArgRefineOvlp);
    ...
    if (opts->use_ksw) os_one_flag_option(kArgUseKsw);
    if (opts->use_replace_mode) os_one_flag_option(kArgUseReplaceMode);
