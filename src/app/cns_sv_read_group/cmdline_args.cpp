...
const int kDfltMemScMemScore = 50;
...
void CommandLineArguments::SetArgumentDescriptions(CArgDescriptions& arg_desc)
{
    ...
    arg_desc.AddDefaultKey(kArgPercentIdentity, "real_value",
                ...
                CArgDescriptions::eDouble,
                NStr::DoubleToString(kDfltCns1PercIdentity));
...
void CommandLineArguments::ExtractAlgorithmOptions(const CArgs& args, CBlastOptions& options)
{
    ...
    if (args.Exist(kArgPercentIdentity) && args[kArgPercentIdentity].HasValue()) {
        m_Options->cns1_perc_identity = args[kArgPercentIdentity].AsDouble();
    }
...
void Init_HbnProgramOptions(HbnProgramOptions* opts)
{
    ...
    opts->cns1_perc_identity = kDfltCns1PercIdentity;
    opts->cns2_perc_identity = kDfltCns2PercIdentity;
...
s_PreCheckCmdLineArgs(int argc, char* argv[], CArgDescriptions* arg_desc)
{
    ...
    string kHbnUsage = kProgram + " [OPTIONS] wrk_dir svr_group_dir db_dir";
...
void ParseHbnProgramCmdLineArguments(int argc, char* argv[], HbnProgramOptions* opts)
{
    ...
    /// query, subject
    if (argc - argv_idx != 3) {
        HBN_ERR("The working dirctory, sv read group directory, and database directory must be specified");
    } 
    opts->wrk_dir = argv[argv_idx];
    opts->svr_group_dir = argv[argv_idx+1];
    opts->db_dir = argv[argv_idx+2];
}
...
char* HbnProgramOptions2String(const HbnProgramOptions* opts)
{
    ...
    os_one_option_value(kArgPercentIdentity, opts->cns1_perc_identity);
