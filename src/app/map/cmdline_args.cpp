...
const string kArgQueryBatchSize("query_batch_size");
const size_t kDfltQueryBatchSize = 500000000;
...
const string kArgSkipMemSc("skip_memsc");
const bool kDfltSkipMemSc = false;
...
void CommandLineArguments::SetArgumentDescriptions(CArgDescriptions& arg_desc)
{
    ...
    arg_desc.AddDefaultKey(kArgQueryBatchSize, "size",
                "Batch size of queries (in bps) processed at one time",
                CArgDescriptions::eDataSize,
                NStr::UInt8ToString_DataSize(kDfltQueryBatchSize));
    ...
    arg_desc.AddFlag(kArgSkipMemSc, "Do not use MEM chaining", true);
...
void CommandLineArguments::ExtractAlgorithmOptions(const CArgs& args, CBlastOptions& options)
{
    ...
    if (args.Exist(kArgQueryBatchSize) && args[kArgQueryBatchSize].HasValue()) {
        m_Options->query_batch_size = NStr::StringToUInt8_DataSize(args[kArgQueryBatchSize].AsString().c_str());
    }
    ...
    if (args.Exist(kArgMaxQueryVolRes) && args[kArgMaxQueryVolRes].HasValue()) {
        ...
        m_Options->max_query_vol_res = NStr::StringToUInt8_DataSize(args[kArgMaxQueryVolRes].AsString().c_str());
    }
    ...
    if (args.Exist(kArgMaxSubjectVolRes) && args[kArgMaxSubjectVolRes].HasValue()) {
        ...
        m_Options->max_subject_vol_res = NStr::StringToUInt8_DataSize(args[kArgMaxSubjectVolRes].AsString().c_str());
    }
    ...
    if (args.Exist(kArgSkipMemSc))
        m_Options->skip_memsc = static_cast<bool>(args[kArgSkipMemSc]);
...
void Init_HbnProgramOptions(HbnProgramOptions* opts, const EHbnTask task)
{
    ...
    /// input query options
    opts->strand = F_R;
    opts->query_batch_size = kDfltQueryBatchSize;
    ...
    /// mem chaining scoring options
    opts->skip_memsc = kDfltSkipMemSc;
...
char* HbnProgramOptions2String(const HbnProgramOptions* opts)
{
    ...
    /// ddf scoring
    if (opts->skip_memsc) os_one_flag_option(kArgSkipMemSc);
