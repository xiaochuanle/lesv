...
int main(int argc, char* argv[])
{
    ...
    HBN_LOG("skip_overhang: %d, perc_identity: %g", opts->skip_overhang, opts->perc_identity);
    ...
    for (int svid = 0; svid < num_subject_vols; svid += subject_vol_stride) {
        ...
        if (all_vs_sj_is_mapped(opts->db_dir, 
                ...
                opts->node_id,
                opts->num_nodes)) {
            merge_all_vs_sj_results(opts->db_dir, kBackupResultsDir, qvid, num_query_vols, svid, opts->node_id, opts->num_nodes, task_struct->out);
            continue;
        }
        ...
        for (; qvid < num_query_vols; qvid += query_vol_stride) {
            if (qi_vs_sj_is_mapped(opts->db_dir, kBackupResultsDir, qvid, svid)) {
                merge_qi_vs_sj_results(opts->db_dir, kBackupResultsDir, qvid, svid, task_struct->out);
                continue;
            }
